var SetEnd, focusElementIfNeeded, getGridProcessorForElement, globalVarForAllGrids, initWiceGrid, isKeySignificant,
  moveDateBoundIfInvalidPeriod, setupAutoreloadsForExternalFilters, setupAutoreloadsForInternalFilters,
  setupBootstrapDatepicker, setupBulkToggleForActionColumn, setupCsvExport, setupDatepicker, setupExternalCsvExport,
  setupExternalSubmitReset, setupHidingShowingOfFilterRow, setupJqueryUiDatepicker, setupMultiSelectToggle,
  setupShowingAllRecords, setupSubmitReset;
...
installed. Either install it pick another filter with :filter_type.`);
  ...
  return $('.date-filter.wg-jquery-datepicker .date-label').each(function(index, removeLink) {
    var dateFormat, datepickerContainer, datepickerHiddenField, eventToTriggerOnChange, labelText, newlyAdded, that,
      yearRange;
